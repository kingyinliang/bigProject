      ...
      <el-card class="searchCard">
        <el-row type="flex">
          ...
          <el-col style="width: 150px">
            ...
            <el-button type="primary" size="small" @click="subAutio()" v-if="isAuth('verify:time:update')">保存</el-button>
      ...
      <el-card class="tableCard">
        ...
        <el-table
          ...
          <el-table-column
            ...
            width="120">
            <template slot-scope="scope">
              <el-select v-model="scope.row.orderNo" placeholder="请选择" size="small" v-if="redactStatus">
                <el-option :label="iteam.value" :value="iteam.code" v-for="(iteam, index) in ARtype" :key="index"></el-option>
              </el-select>
              <el-select v-model="scope.row.orderNo" placeholder="请选择" size="small" v-else disabled>
                <el-option :label="iteam.value" :value="iteam.code" v-for="(iteam, index) in ARtype" :key="index"></el-option>
              </el-select>
            </template>
          </el-table-column>
          ...
          <el-table-column
            ...
            width="120">
            <template slot-scope="scope">
              <el-select v-model="scope.row.deptId" placeholder="请选择" size="small" v-if="redactStatus">
          ...
          <el-table-column
            ...
            width="120">
            <template slot-scope="scope">
              <el-select v-model="scope.row.userType" placeholder="请选择" size="small" v-if="redactStatus">
          ...
          <el-table-column
            ...
            width="120">
            <template slot-scope="scope">
              <el-input v-model="scope.row.orderNo" v-if="redactStatus" size="small" placeholder="手工录入"></el-input>
...
export default {
  ...
  methods: {
    ...
    // 获取考勤属性
    GetARpro () {
      this.$http(`${SYSTEMSETUP_API.PARAMETERLIST_API}?type=KQ_ATT`, 'POST').then(({data}) => {
        if (data.code === 0) {
    ...
    addAR () {
      if (this.plantList.workShop === '') {
        ...
        this.datalist.push({workShop: this.plantList.workShop, userType: '', deptId: '', userId: []})
      }
    },
    // 获取列表
    GetList (st) {
      this.clearStatus = true
      this.datalist = [{orderNo: '22', userType: '', deptId: ''}, {orderNo: '22', userType: '', deptId: ''}]
      return false
      if (st) {
