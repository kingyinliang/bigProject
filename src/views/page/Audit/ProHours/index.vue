        ...
        <el-card class="tableCard">
          ...
          <el-table
            ...
            <el-table-column
              ...
              width="120">
              <template slot-scope="scope">
                <el-button class="ra_btn" type="primary" round size="mini" @click="redact(scope.row)" v-if="!((scope.row.status === 'checked' && scope.row.interfaceReturnStatus === '1') || scope.row.status === 'noPass') && isAuth('verify:time:update')">{{ scope.row.redact? '保存' : '编辑'}}</el-button>
                <el-button class="ra_btn" type="warning" round size="mini" @click="ResetD(scope.row)" v-if="scope.row.status === 'checked' && scope.row.interfaceReturnStatus === '1' && isAuth('verify:time:resetTime')">反审</el-button>
              </template>
            </el-table-column>
          </el-table>
          <el-row >
