...
export default {
  ...
  methods: {
    getOrderList () {
      ...
      if (this.params.type === '') {
        this.$notify.error({title: '错误', message: '请选择类型'})
        return
      }
    ...
    ProcessingData (data) {
      ...
      data.forEach((item, index) => {
        if (this.params.type === '投料') {
          if (item.outStatus === 'noPass' || item.outStatus === 'saved' || item.outStatus === '已同步') {
            newArr.push(item)
          }
      ...
      } else if (this.params.type === '投料') {
        newArr.sort(function (a, b) {
          var order = ['noPass', '已同步', 'saved']
